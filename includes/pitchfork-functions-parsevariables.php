...
{ # Define the default output variable...
	$output = "Parse error: ". $variable . " not recognized.";
		...
		{$temp_string = str_replace("[Pitchfork:UI:CSS:", "", $variable);
		 $temp_string = str_replace("]", "", $temp_string);
		 $temp_string .= '.css';
		 $this->include_css($temp_string);}
		 
		 elseif(substr_count($variable, "[String:"))
		 {
		 	$temp_string = str_replace("[String:","",$variable);
		 	$temp_string = str_replace("]","",$temp_string);
		 	$output = $this->string($temp_string, false);
		 }
